...
dataframe <- dataframe %>%
    ...
    group_by(week = id %/% 7) |> 
    mutate(across(c(where(is.numeric), -day, -id), \(x) mean(x, na.rm = TRUE),  .names = "week_mean_{.col}")) |>
    ungroup() |>
...
A benefit of this technique is noise reduction, at least at the 2 percent most anomylous observations.  A drawback is of applying it at this point to the training data and the cross validation folds is that the CV out of sample estimates of error are smaller than we would see with raw.
                   
Computationally, it is faster to run it here. The method relies on the `ranger`                    
...
lagged_rec <- recipe(
  ...
  step_mutate(delta_pressure = lag_1_pressure - pressure) |>
...
dep_models <- dep_models |> 
  ...
  option_add(
   ...
   workflow_map("tune_grid", resamples = folds, grid = 5, 
