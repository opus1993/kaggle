...
preprocessor <- function(dataframe) {
  dataframe %>%
    ...
    mutate(height_outlier = factor(if_else(!between(height, 0.0001, 0.3), "reasonable_height","holy_crap_this_is_not_right_height")))
}
...
#     mtry = mtry(range = c(5,9)),
      ...
      tree_depth = tree_depth(range = c(15, 30)),
      learn_rate = learn_rate(range = c(-0.4, -0.6)),
...
augment(fit2, raw_df) %>% ggplot(aes(rings, .pred - rings, color = sex)) + geom_jitter() +
  labs(title = "With Outforest Anomaly Imputation")              
