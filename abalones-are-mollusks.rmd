...
So many of the bank balance observations are zero. I will add a flag. I wonder if these model less well than those without the flag.
...
Nominal features `r nom_features`

Numeric features `r num_features`
...
raw_df %>%
  GGally::ggpairs(
    mapping = aes(color = ggplot2::cut_number(rings,5)),
    lower = list(
...
I will come back and run checks on how the train data and the test data are different.                   
...
param <- wf %>%
   ...
   recipes::update(
     mtry = mtry(range = c(5,9)),
      ...
      tree_depth = tree_depth(range = c(6, 25)),
     # learn_rate = learn_rate(range = c(-1, -2.5)),
      num_leaves = num_leaves(range = c(30, 70))
   ) %>%
...
submit_df <- augment(fit, competition_df) %>% 
