...
remotes::install_github("MatthewBJane/ThemePark", quiet = TRUE)    
...
theme_set(theme_spiderman())

#hexsession::make_tile(packages = c("bonsai", "catboost", "DALEXtra", "DALEX",  "correlationfunnel","yardstick","workflows",
#                                   "tune","rsample","recipes","parsnip","dials","broom","tidymodels","lubridate","ThemePark",
...
dataframe <- dataframe |>
  ...
  mutate(gap_value = 
           case_when(
             contract == "Month-to-month" ~ total_charges -  1/12 * monthly_charges,
             contract == "One year" ~ total_charges -  12 * monthly_charges,
             TRUE ~ total_charges - 24 * monthly_charges
           )  ) |> 
...
features <- train_df %>%
  dplyr::select(-id, -churn, -factor_id) |> 
  names()
...
#  scale_fill_brewer(type = "qual", palette = "Dark2") +                   
  ...
  theme_void() +
  labs(title = "Customer Churn", caption = "Data: Kaggle.com | Visual: Jim Gruman")
...
train_df <- train_df |> predict(read_rds("/kaggle/working/classification_fit.rds"), train_df, type = "prob") |>  
      transmute(past_run = .pred_No)  |> 
      bind_cols(train_df)

rec <- recipe(formula(paste0("churn ~ ", str_c(features, "past_run", collapse = " + "))), data = train_df)   |> 
  step_nzv(all_predictors()) |> 
  step_lincomb(all_numeric_predictors()) |>
  embed::step_woe(gender, partner, outcome = vars(churn))
...
rec |> prep() |> bake(new_data = NULL) |> skimr::skim()
...
catboost_params <- dep_models |>
 ...
 update(
         ...
         learn_rate = learn_rate(range = c(-1.3,-0.9))
 ) 
...
write_rds(butcher::butcher(classification_fit, verbose = TRUE), "/kaggle/working/classification_fit.rds")
                   
predict(classification_fit, train_df, type = "prob") %>% 
  bind_cols(train_df) |> 
  mutate(
    .pred_class = factor(if_else(.pred_No < 0.5, "Yes","No"))
  ) |>                    
...
predict(classification_fit, train_df, type = "prob") %>% 
  bind_cols(train_df) |> 
  yardstick::roc_auc(churn, .pred_No)
