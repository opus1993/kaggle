---
title: "Paris home values Season3 Episode6"
...
isFactor <-c("has_yard","has_pool","is_new_built","has_storm_protector","has_storage_room","has_guest_room","city_code")

preprocessor <- function(data){
  data |> 
    janitor::clean_names() %>% 
    mutate(across(all_of(isFactor), factor)) %>% 
    filter(id != 15334) %>% # square meters outlier
    filter(id != 5659) %>% # floor outlier  
    filter(made < 2025) %>% 
    filter(garage < 1001)
}
...
raw_df |> 
  ...
  ggplot(aes(value, fill = metric, color = NULL)) +
  geom_histogram(alpha = 0.6, bins = 30, show.legend = FALSE) +
...
workflow(rec, boost_tree_xgboost_spec) %>%
  ...
  labs(title = "XGBboost Variable Importance")
